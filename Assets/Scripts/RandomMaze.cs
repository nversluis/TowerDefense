...
public class RandomMaze : MonoBehaviour
{
	...
	private void spawnTorch (float n, float e,int angle){
		torchCount++;
		if (torchCount % 1 == 0) {
			GameObject torchObj = (GameObject)Instantiate (torch, new Vector3 (n * planewidth, height * planewidth * 0.5f, e * planewidth), Quaternion.Euler (0, angle, 0));
		}
	}
		...
		for (int i = 0; i < positions.Count; i++) {;
			...
			for (float n = nstart; n <= planewidth - nend; n = n + nodeSize) {
				...
				for (float e = estart; e <= planewidth - eend; e = e + nodeSize) {
					...
					Vector3 nodeposition = new Vector3 (curPos [0] * planewidth + (n - planewidth / 2), 0, curPos [1] * planewidth + (e - planewidth / 2));
					if (!NodesPos.Contains (nodeposition)) {
						NodesPos.Add (nodeposition); //Add instantiated node position to NodePos
						Nodes.Add (new WayPoint (nodeposition)); //Add Instantiated node as waypoint to Nodes
					}
				}
			}

			for (float n = nstart + 1; n <= planewidth - nend - 1; n = n + nodeSize) {

				for (float e = estart + 1; e <= planewidth - eend - 1; e = e + nodeSize) {
